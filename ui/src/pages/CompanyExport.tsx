...
/**
 * Filter .ai-ceo.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterAICEOYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://ai-ceo.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm ai-ceo company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [AI CEO](https://ai-ceo.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [AI CEO](https://ai-ceo.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .ai-ceo.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .ai-ceo.yaml
    const yamlPath = exportData.aiCeoExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterAICEOYaml(exportData.files[yamlPath], checkedFiles);
    }
