...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="AI CEO API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "aiCeoApiUrl",
                  String(config.aiCeoApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "aiCeoApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://ai-ceo.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "ai-ceo"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. AI CEO persists a device key during onboarding so pairing approvals
