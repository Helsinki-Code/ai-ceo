 ...
 *
 * Plugin bundles are built with `external: ["@ai-ceo/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__aiCEOPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__aiCEOPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__aiCEOPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__aiCEOPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@ai-ceo/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@ai-ceo/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@ai-ceo/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@ai-ceo/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@ai-ceo/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__aiCEOPluginBridge__) {
 ...
 * self-register via `window.aiCeoPlugins.registerReactComponent()`.
 ...
 * of `@ai-ceo/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
