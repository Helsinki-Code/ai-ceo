...
AI CEO V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a AI CEO-shaped design.

Assumption for this document: AI CEO is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class AI CEO objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into AI CEO.

The main conclusion is:

- AI CEO should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- AI CEO should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- AI CEO should use multiple extension classes instead of one generic plugin bag:
...
If AI CEO does this well, the examples you listed become straightforward:
...
Relevant AI CEO files reviewed for current extension seams:
...
It is too dangerous for AI CEO core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for AI CEO — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
AI CEO should absolutely do this.
...
AI CEO should copy this discipline.
...
AI CEO should adopt this style for plugin actions, automations, and UI schemas.
...
AI CEO can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for AI CEO too.

## What AI CEO Should Not Copy Directly
...
AI CEO is a control plane for an operator-managed instance with company objects.
...
AI CEO is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

AI CEO should not auto-load arbitrary code from a workspace repo like `.ai-ceo/plugins` or project directories.
...
For AI CEO, equivalent hooks into:
...
AI CEO should not allow plugins to silently replace:
...
For AI CEO it would be risky because it combines:
...
AI CEO should require an explicit operator install step.

## Why AI CEO Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | AI CEO |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted AI CEO instance |
...
That means AI CEO should borrow the good ideas from `opencode` but use a stricter architecture.

## AI CEO Already Has Useful Pre-Plugin Seams

AI CEO has several extension-like seams already:
...
This is good news.
AI CEO does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended AI CEO Plugin Model
...
AI CEO already has this pattern implicitly:
...
- `plugin` means integration code that talks to AI CEO through a typed plugin protocol and capability model
...
- invoke explicit AI CEO actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@ai-ceo/plugin-sdk/ui`) with design tokens and common components.
...
AI CEO should treat plugin installation as a global instance-level action.

Examples:

- install `@ai-ceo/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over AI CEO objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

AI CEO already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for AI CEO too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@ai-ceo/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@ai-ceo/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running AI CEO instance.

A `create-ai-ceo-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how AI CEO achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the AI CEO server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. AI CEO's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@ai-ceo/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@ai-ceo/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For AI CEO

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@ai-ceo/plugin-sdk";

export default definePlugin({
  id: "@ai-ceo/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit AI CEO entities
    });
    ...
    ctx.events.on("plugin.@ai-ceo/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@ai-ceo/plugin-sdk/ui";
...
Any AI CEO plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific AI CEO objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@ai-ceo/plugin-linear`)
...
- reuse core tables when the data is clearly part of AI CEO itself
...
If a concept is becoming part of AI CEO's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of AI CEO itself
...
### 3. Scope plugin data to AI CEO objects before adding custom schemas

A lot of plugin data naturally hangs off existing AI CEO objects:
...
If AI CEO eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@ai-ceo/plugin-workspace-files`
...
Package idea: `@ai-ceo/plugin-terminal`
...
Package idea: `@ai-ceo/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving AI CEO.
- Board reviews the diff after a run without leaving AI CEO.
...
- optional `events.emit` (e.g. `plugin.@ai-ceo/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@ai-ceo/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@ai-ceo/plugin-linear`

This plugin syncs AI CEO work with Linear. It is useful for:

- importing backlog from Linear
- linking AI CEO issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between AI CEO and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into AI CEO.
- AI CEO issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@ai-ceo/plugin-github-issues`

This plugin syncs AI CEO issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to AI CEO issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with AI CEO task governance
...
- Plugin settings:
  ...
  - whether new AI CEO issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active AI CEO issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into AI CEO.
- GitHub webhooks update status/comment state in AI CEO.
- A PR is linked back to the AI CEO issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@ai-ceo/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@ai-ceo/plugin-grafana`

This plugin surfaces external metrics and dashboards inside AI CEO. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating AI CEO issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full AI CEO plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the AI CEO dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a AI CEO issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of AI CEO core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@ai-ceo/plugin-runtime-processes`
...
Package idea: `@ai-ceo/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into AI CEO. It is useful for:
...
- Failed payment spikes or churn events can generate AI CEO issues for follow-up.
...
- Stripe data should stay additive to AI CEO core
...
- plugin UI bundle loading, host bridge, `@ai-ceo/plugin-sdk/ui`
...
- `@ai-ceo/plugin-test-harness` and `create-ai-ceo-plugin` starter template
...
AI CEO should not implement "an OpenCode-style generic in-process hook system."
AI CEO should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In AI CEO
...
8. Ship `@ai-ceo/plugin-test-harness` and `create-ai-ceo-plugin` starter template.
