# AI CEO Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for AI CEO's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main AI CEO app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary AI CEO HTTP APIs directly.
...
AI CEO plugin design is based on the following assumptions:

1. AI CEO is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core AI CEO business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by AI CEO core.
...
2. Let plugins add major capabilities without editing AI CEO core.
...
4. Depend on project-local plugin folders such as `.ai-ceo/plugins`.
...
The single AI CEO deployment an operator installs and controls.

### 5.2 Company

A first-class AI CEO business object inside the instance.
...
A trusted in-process extension loaded directly by AI CEO core.
...
An installable instance-wide extension package loaded through the AI CEO plugin runtime.
...
AI CEO has two extension classes.
...
AI CEO already has a concrete workspace model:
...
Plugins live under the AI CEO instance directory.

Suggested layout:

- `~/.ai-ceo/instances/default/plugins/package.json`
- `~/.ai-ceo/instances/default/plugins/node_modules/`
- `~/.ai-ceo/instances/default/plugins/.cache/`
- `~/.ai-ceo/instances/default/data/plugins/<plugin-id>/`
...
AI CEO should add CLI commands:

- `pnpm ai-ceo plugin list`
- `pnpm ai-ceo plugin install <package[@version]>`
- `pnpm ai-ceo plugin uninstall <plugin-id>`
- `pnpm ai-ceo plugin upgrade <plugin-id> [version]`
- `pnpm ai-ceo plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@ai-ceo/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
Plugins may contribute tools that AI CEO agents can use during runs.
...
- AI CEO server starts one worker process per installed plugin
...
Receives one typed AI CEO domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@ai-ceo/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@ai-ceo/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@ai-ceo/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@ai-ceo/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@ai-ceo/plugin-sdk/ui`
...
The `@ai-ceo/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the AI CEO secret provider system rather than a plain text input
...
1. Core AI CEO data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing AI CEO objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual AI CEO product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if AI CEO later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing AI CEO secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm ai-ceo plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the AI CEO server. This is a normative requirement, not optional.
...
### 27.1 `@ai-ceo/plugin-test-harness`
...
```ts
import { createTestHarness } from "@ai-ceo/plugin-test-harness";
...
For developing a plugin against a running AI CEO instance:

- The operator installs the plugin from a local path: `pnpm ai-ceo plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-ai-ceo-plugin`) that scaffolds:
...
- `@ai-ceo/plugin-workspace-files`
- `@ai-ceo/plugin-terminal`
- `@ai-ceo/plugin-git`
- `@ai-ceo/plugin-linear`
- `@ai-ceo/plugin-github-issues`
- `@ai-ceo/plugin-grafana`
- `@ai-ceo/plugin-runtime-processes`
- `@ai-ceo/plugin-stripe`
...
- `@ai-ceo/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@ai-ceo/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@ai-ceo/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@ai-ceo/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@ai-ceo/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@ai-ceo/plugin-sdk/ui`
...
- `@ai-ceo/plugin-test-harness`
- `create-ai-ceo-plugin` starter template
...
AI CEO should not implement a generic in-process hook bag modeled directly after local coding tools.

AI CEO should implement:
...
That is the complete target design for the AI CEO plugin system.
