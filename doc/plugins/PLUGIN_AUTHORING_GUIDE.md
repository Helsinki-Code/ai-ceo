...
This guide describes the current, implemented way to create a AI CEO plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main AI CEO app.
...
```bash
pnpm --filter @ai-ceo/create-ai-ceo-plugin build
node packages/plugins/create-ai-ceo-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the AI CEO repo:

```bash
pnpm --filter @ai-ceo/create-ai-ceo-plugin build
node packages/plugins/create-ai-ceo-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/ai-ceo/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@ai-ceo/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@ai-ceo/plugin-sdk` from the local AI CEO checkout into a `.ai-ceo-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into AI CEO from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@ai-ceo/plugin-sdk/ui`
