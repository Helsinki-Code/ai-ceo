...
This spec defines how AI CEO actually runs agents while staying runtime-agnostic.
...
1. AI CEO is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, AI CEO manages lifecycle, and on exit AI CEO parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `ai-ceo.skill` (shared AI CEO skill text block)
...
2. Preferred transport is env vars (`AI_CEO_*`) injected at runtime.
