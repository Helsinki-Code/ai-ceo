...
For `claude_local` and `codex_local`, AI CEO assumes the CLI is already installed and authenticated on the host machine.
...
AI CEO stores resumable session state per `(agent, taskKey, adapterType)`.
