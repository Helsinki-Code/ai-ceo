...
Low-level reference for how AI CEO packages are prepared and published to npm.
...
AI CEO no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `ai-ceo`, imports code from workspace packages such as:
...
### Manual first publish for `@ai-ceo/ui`
...
npm view @ai-ceo/ui version

# make sure the dist payload is fresh
pnpm --filter @ai-ceo/ui build
...
- If `npm view @ai-ceo/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@ai-ceo/ui`, the `ai-ceo` npm organization must exist and the publisher must be a member with permission to publish to that scope.
