...
Add a single storage subsystem for AI CEO that supports:
...
- API serves attachment bytes through authenticated AI CEO endpoints.
...
- Default local path is under instance root: `~/.ai-ceo/instances/<instanceId>/data/storage`.
...
- `ai-ceo onboard` writes a valid `storage` config block by default.
- `ai-ceo configure --section storage` can switch between local and s3 modes.
...
- `ai-ceo doctor` reports actionable storage status.
