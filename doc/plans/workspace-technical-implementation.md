...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/ai-ceo-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `AI_CEO_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `AI_CEO_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - AI CEO may create host-local worktrees, processes, and paths
- `cloud`
  - AI CEO should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `ai-ceo | github | vercel | s3 | custom`
...
- use `AI_CEO_EXECUTION_TOPOLOGY` as a defaulting guardrail
