...
AI CEO's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends AI CEO's API, UI, and data model | Yes |
...
modules/
  observability/
    ai-ceo.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@ai-ceo/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@ai-ceo/core";
...
```
1. Scan modules/ directory for ai-ceo.module.json manifests
...
```jsonc
// ai-ceo.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@ai-ceo/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@ai-ceo/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:ai-ceo/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.ai-ceo.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm ai-ceo store list                    # browse available modules and templates
pnpm ai-ceo store install <module-id>     # install a module
pnpm ai-ceo store import <template-id>    # import a company template
pnpm ai-ceo store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one AI CEO instance | `server:started` |
...
4. **Module config** — `ai-ceo.config.json` with per-module config, env var interpolation
...
10. **`@ai-ceo/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
