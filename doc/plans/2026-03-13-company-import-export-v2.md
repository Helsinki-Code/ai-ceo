...
This document defines the next-stage plan for AI CEO company import/export.

The core shift is:

- move from a AI CEO-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside AI CEO.
...
AI CEO already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just AI CEO
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `ai-ceo.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.ai-ceo.yaml` sidecar for high-fidelity AI CEO-specific details
...
11. adapter-aware skill sync surfaces so AI CEO can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point AI CEO at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a AI CEO sidecar.
...
AI CEO must not redefine `SKILL.md`.
...
- AI CEO-specific fidelity lives in `.ai-ceo.yaml`
- AI CEO may resolve and install `SKILL.md` packages, but it must not require a AI CEO-only skill format
...
### 5.4 Base Package Vs AI CEO Extension
...
- base package:
  ...
  - no AI CEO-only runtime fields by default
- AI CEO extension:
  - `.ai-ceo.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by AI CEO tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`ai-ceo.manifest.json` is not part of the future package direction.
...
AI CEO import/export should support these entity kinds:
...
- AI CEO can treat “this set of agents came from team package X” as the imported-team model
...
AI CEO should support source references in package metadata with:
...
AI CEO should define a new adapter surface area around skills:
...
Planned AI CEO behavior:

- if an adapter supports read, AI CEO should show current skills in the UI
- if an adapter supports write, AI CEO should let the user enable/disable imported skills
- if an adapter supports sync, AI CEO should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.ai-ceo.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.ai-ceo.yaml` alongside the base package
...
## 10. Storage Model Inside AI CEO
...
AI CEO should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `ai-ceo company export <company-id> --out <path>`
- `ai-ceo company import <path-or-url> --dry-run`
- `ai-ceo company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in AI CEO storage, or only referenced at import time?
...
3. Should AI CEO support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps AI CEO aligned with:
