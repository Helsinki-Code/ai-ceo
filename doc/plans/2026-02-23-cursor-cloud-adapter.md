...
This document defines the V1 design for a AI CEO adapter that integrates with
...
4. synthesized stdout events for AI CEO UI/CLI
...
3. **Agent callback auth to AI CEO** uses a bootstrap exchange flow (no long-lived AI CEO key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from AI CEO endpoints, not full SKILL.md prompt injection.
...
- `paperclipPublicUrl` (optional override; else `AI_CEO_PUBLIC_URL` env)
...
## AI CEO Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `AI_CEO_API_KEY`.
...
2. `process.env.AI_CEO_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived AI CEO credentials in prompt text.

Flow:

1. Before launch/follow-up, AI CEO mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. AI CEO validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all AI CEO API calls.
...
1. Prompt includes a compact instruction to fetch skills from AI CEO.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/ai-ceo`
   - `GET /api/skills/ai-ceo-create-agent` when needed
...
4. Route by Cursor `agentId` to active AI CEO run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from AI CEO skill endpoints |
| AI CEO auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from AI CEO context.
3. Add richer PR metadata surfacing in AI CEO UI.
