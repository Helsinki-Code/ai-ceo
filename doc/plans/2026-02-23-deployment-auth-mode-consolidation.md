...
Keep AI CEO low-friction while making the mode model simpler and safer:
...
1. `pnpm ai-ceo onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm ai-ceo doctor` works flagless with mode-aware checks.
