...
AI CEO needs to support two very different but equally valid ways of working:
...
Today, AI CEO already has the beginnings of this model:
...
- making AI CEO feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing AI CEO users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted AI CEO deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning AI CEO into a full code review product
...
This object must be recorded explicitly so that AI CEO can:
...
AI CEO should treat PRs as a type of work product linked back to:
...
AI CEO already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted AI CEO must be a first-class deployment mode

AI CEO cannot assume that it is running on the same machine as the code.

In cloud deployments, AI CEO may:
...
- `Runtime service`: a process or service AI CEO owns or tracks for a workspace
...
It also lets cloud-hosted AI CEO deployments point at codebases and remotes without pretending the AI CEO host has direct filesystem access.
...
Without an explicit `execution workspace` record, AI CEO has nowhere stable to attach:
...
AI CEO needs a single place to show:
...
- AI CEO-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without AI CEO owning the execution host
...
## 1. AI CEO host and execution host must be decoupled
...
without the AI CEO host itself running local git or local preview processes.
...
AI CEO should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for AI CEO.
...
AI CEO should keep workspace automation centered on a small set of hooks:
...
AI CEO should provide the equivalent visibility in the project `Code` surface:
...
AI CEO should treat execution workspaces as the natural home for:
...
AI CEO should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
AI CEO should continue treating cleanup policy as part of the core workspace model:
...
AI CEO should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the AI CEO host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

AI CEO should borrow Worktrunk's good execution patterns while keeping the broader AI CEO model:
...
That keeps AI CEO focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted AI CEO deployments remain first-class

That is a better fit for AI CEO than either extreme:
