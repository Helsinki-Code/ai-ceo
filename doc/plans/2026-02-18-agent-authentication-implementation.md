...
- New config/env settings:
  - `AI_CEO_AGENT_JWT_SECRET`
  - `AI_CEO_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `AI_CEO_AGENT_JWT_ISSUER` (default: `ai-ceo`)
  - `AI_CEO_AGENT_JWT_AUDIENCE` (default: `ai-ceo-api`)
...
1. In:
   ...
   inject `AI_CEO_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `AI_CEO_API_KEY`, do not overwrite it.
- Continue injecting:
  - `AI_CEO_AGENT_ID`
  - `AI_CEO_COMPANY_ID`
  - `AI_CEO_API_URL`
...
- Local adapters authenticate without manual `AI_CEO_API_KEY` config.
