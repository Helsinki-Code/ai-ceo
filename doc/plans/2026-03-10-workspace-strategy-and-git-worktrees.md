...
`PAP-447` asks how AI CEO should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same AI CEO instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into AI CEO:

- some operators use AI CEO to manage AI CEO and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as AI CEO
- Claude and Codex expose different built-in affordances, so AI CEO should not overfit to one tool

## Core Product Decision

AI CEO should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by AI CEO
...
- AI CEO should not assume that every adapter can see or use a host filesystem path directly
...
If AI CEO makes worktrees agent-first, it will blur:
...
- AI CEO core resolves and records execution workspace state
...
That gives AI CEO room to support:
...
- AI CEO handles workspace runtime services under the hood using project/workspace policy
...
Once AI CEO is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that AI CEO cannot infer
...
- when issue moves to done, should AI CEO auto-commit?
- should it auto-push?
...
AI CEO already has the right foundation for a project-first model.
...
For server-spawned local adapters, AI CEO already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that AI CEO should mirror directly

Implication:

- for `codex_local`, AI CEO should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but AI CEO should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, AI CEO should separate:
...
AI CEO should be able to express intentions such as:
...
These are not specific to AI CEO. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".ai-ceo/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a AI CEO-specific `devServer` flag.
...
AI CEO should distinguish between:
...
AI CEO should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, AI CEO should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to AI CEO
...
For runtime services specifically, AI CEO should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as AI CEO-only dev-server flags
...
- same embedded DB / AI CEO instance can be reused through env/config injection when appropriate
...
- AI CEO can decide whether to reuse or start a fresh service deterministically
...
3. Test against AI CEO-on-AI CEO workflows first.
...
7. Workspace runtime services are modeled generically, not as AI CEO-specific dev-server toggles.
