...
- local mode that still feels instant (`npx ai-ceo run` and go)
...
2. CLI command `pnpm ai-ceo auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm ai-ceo onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm ai-ceo onboard`).
...
- expose key instance settings in API + CLI (`ai-ceo configure` / `ai-ceo onboard`)
...
7. `pnpm ai-ceo onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
