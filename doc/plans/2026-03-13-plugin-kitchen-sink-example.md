...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented AI CEO plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside AI CEO.
...
- package: `@ai-ceo/plugin-kitchen-sink-example`
- plugin id: `ai-ceo.kitchen-sink-example` or `ai-ceo-kitchen-sink-example`

Recommendation: use `ai-ceo-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core AI CEO event such as `issue.created`
...
Recommendation: default to a AI CEO-local endpoint or a stable public echo endpoint to avoid flaky docs.
