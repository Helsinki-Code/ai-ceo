...
This document defines the product and UI plan for skill management in AI CEO.
...
- AI CEO company import/export can include skills as package content
...
3. The UI must always tell the truth about what AI CEO knows:
   - desired state in AI CEO
...
AI CEO should treat skills at two scopes:
...
- created locally inside AI CEO later
...
7. “Understand what AI CEO knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a AI CEO-specific package layout
...
This matters because Codex and similar adapters may already have local skills that AI CEO did not install.

These should be clearly marked:

- external
- not managed by AI CEO
...
- `desired_only`: AI CEO wants it, adapter does not show it yet
- `external`: adapter has it but AI CEO does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, AI CEO only tracks desired state
...
- AI CEO can still track desired skills
...
- `.ai-ceo.yaml` may add AI CEO-specific fidelity, but should not replace the base shortname association model
...
- `.ai-ceo.yaml` may add AI CEO-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
AI CEO should have a company-scoped skill model or managed package model representing:
...
AI CEO should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too AI CEO-specific.
...
That gives AI CEO one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
