...
We need evals for the thing AI CEO actually ships:
...
- can we grow the suite over time from real AI CEO usage?
...
- Discussion #449: <https://github.com/ai-ceo/ai-ceo/discussions/449>
...
AI CEO should take a **two-stage approach**:

1. **Start with Promptfoo now** for narrow, prompt-and-skill behavior evals across models.
2. **Grow toward a first-party, repo-local eval harness in TypeScript** for full AI CEO scenario evals.
...
3. The longer-term harness should run **real AI CEO scenarios** against seeded companies/issues/agents, not just raw prompt completions.
...
That is the right unit because that is what actually changes behavior in AI CEO.

## Why This Is The Right Shape

### 1. We need to evaluate system behavior, not only prompt output

Prompt-only tools are useful, but AI CEO’s real failure modes are often:
...
But OpenAI’s Evals API is not the right control plane for AI CEO as the primary system because our target is explicitly multi-model and multi-provider.
...
But AI CEO should still avoid making a hosted platform or a third-party config format the core abstraction before we have our own stable eval model.

The right move is:

- start with Promptfoo for quick wins
- keep the data portable and repo-owned
- build a thin first-party harness around AI CEO concepts as the system grows
...
These are the closest thing to prompt evals, but still framed in AI CEO terms.
...
The important part is that the case is about a AI CEO scenario, not a standalone prompt string.
...
AI CEO should grow eval coverage by promoting real runs into cases when we see:
...
- AI CEO needs scenario setup, control-plane state inspection, and multi-step traces as first-class concepts

Recommendation:

- use Promptfoo first
- store Promptfoo config and cases in-repo under `evals/promptfoo/`
- use custom JS/TS assertions and, if needed later, a custom provider that calls AI CEO scenario runners
- do not make Promptfoo YAML the only canonical AI CEO eval format once we outgrow prompt-level evals
...
- AI CEO must compare across models/providers
- we do not want our primary eval runner coupled to one model vendor

Recommendation:

- use the guidance
- do not use it as the core AI CEO eval runtime
...
- did they call the right AI CEO APIs?
...
Start with Promptfoo for immediate, narrow model-and-prompt comparisons, then grow into a first-party `evals/` framework in TypeScript that evaluates **AI CEO scenarios and bundles**, not just prompts.
...
But keep the canonical eval model inside the AI CEO repo and aligned to AI CEO’s actual control-plane behaviors.
