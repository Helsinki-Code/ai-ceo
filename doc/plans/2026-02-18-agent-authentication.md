...
Agents need API keys to authenticate with AI CEO. The current approach
...
**Trust model:** The adapter process runs on the same machine as the AI CEO
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** AI CEO generates a token and passes it directly to the agent
...
`AI_CEO_API_URL`, `AI_CEO_AGENT_ID`, `AI_CEO_COMPANY_ID`. We need to
add a `AI_CEO_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a AI CEO CLI
...
```
ai-ceo auth login
# Opens browser -> user confirms -> token stored at ~/.ai-ceo/credentials
```
...
managed by the AI CEO server itself.
...
1. A company admin (user or agent) generates an **invite URL** from AI CEO.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The AI CEO SKILL.md (or a link to it)
   - What information AI CEO needs from the agent (e.g. webhook URL, adapter
...
5. AI CEO stores the pending registration.
...
7. On approval, AI CEO provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** AI CEO issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.ai-ceo.ing/skills/ai-ceo/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How AI CEO should send you heartbeats",
...
1. Generate a AI CEO invite link for the company.
...
4. A AI CEO company member approves the new agent.
5. AI CEO begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `AI_CEO_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `ai-ceo auth login` for developer-managed remote agents.                                      |
