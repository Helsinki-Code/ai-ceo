...
ClipHub is the public registry where people share, discover, and download AI CEO company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to AI CEO what a package registry is to a programming language. AI CEO already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in AI CEO — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own AI CEO instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the AI CEO spec. It contains:
...
**Install (fresh start):**
```
ai-ceo install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local AI CEO instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within AI CEO, export your company as a template, then publish:

```
ai-ceo export --template my-company
ai-ceo publish cliphub my-company
```
...
ClipHub is a **separate service** from AI CEO itself. AI CEO is self-hosted; ClipHub is a hosted registry that AI CEO instances talk to.
...
| **AI CEO CLI** | `ai-ceo install`, `ai-ceo publish`, `ai-ceo cliphub sync` — built into AI CEO |
| **AI CEO UI** | "Browse ClipHub" panel in the AI CEO web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with AI CEO) |
| Backend | TypeScript + Hono (consistent with AI CEO) |
| Database | PostgreSQL |
...
4. Run `ai-ceo install cliphub:acme/lean-saas-shop`
5. AI CEO creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in AI CEO until it works well
2. Export: `ai-ceo export --template my-agency`
3. Publish: `ai-ceo publish cliphub my-agency`
...
3. Install just that agent: `ai-ceo install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to AI CEO

ClipHub is **not required** to use AI CEO. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to AI CEO what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `ai-ceo install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in AI CEO web UI
- [ ] `ai-ceo cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with AI CEO's own philosophy)
