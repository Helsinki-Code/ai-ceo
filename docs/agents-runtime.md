...
Audience: Operators setting up and running agents in AI CEO

## 1. What this system does

Agents in AI CEO do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`), AI CEO assumes the CLI is already installed and authenticated on the host machine.
...
AI CEO stores session IDs for resumable adapters.
...
AI CEO pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. AI CEO surfaces this as a warning in environment tests, not a hard error.
