...
How to get OpenClaw running in a Docker container for local development and testing the AI CEO OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

AI CEO includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-ai-ceo-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a AI CEO host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-ai-ceo-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `AI_CEO_HOST_PORT` (default `3100`)
- `AI_CEO_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your AI CEO deployment is `authenticated`, provide auth context:

```bash
AI_CEO_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
AI_CEO_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host AI CEO server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed AI CEO URL (typically `http://host.docker.internal:3100`).
- If AI CEO rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm ai-ceo allowed-hostname host.docker.internal
```

Then restart AI CEO and rerun the smoke script.
