...
AI CEO companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .ai-ceo.yaml     # Adapter config, env inputs, routines
```
...
- **.ai-ceo.yaml** holds AI CEO-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
ai-ceo company export <company-id> --out ./my-export
```
...
ai-ceo company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
ai-ceo company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
ai-ceo company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.ai-ceo.yaml`
...
```sh
# From a local folder
ai-ceo company import ./my-export

# From a GitHub URL
ai-ceo company import https://github.com/org/repo

# From a GitHub subfolder
ai-ceo company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
ai-ceo company import org/repo
ai-ceo company import org/repo/companies/acme
```
...
If `--target` is not specified, AI CEO infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
ai-ceo company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
ai-ceo company import org/company-templates/engineering-team \
  --target new \
...
```sh
ai-ceo company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
ai-ceo company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
ai-ceo company import ./package \
  --target new \
  --yes \
  --json
```
...
AI CEO supports several GitHub URL formats:
