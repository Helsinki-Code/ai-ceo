# ClipHub: Marketplace for AI CEO Team Configurations
...
> The "app store" for whole-company AI teams — pre-built AI CEO configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for AI CEO-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | AI CEO company (agents, projects, governance) |
...
A complete AI CEO company configuration:
...
Single-agent configurations designed to plug into a AI CEO org:
...
Portable skill files that any AI CEO agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with AI CEO's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum AI CEO version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target AI CEO company
...
| `POST` | `/api/listings/:id/install` | Install to AI CEO company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target AI CEO company (or create new)
  → ClipHub API calls AI CEO API to:
  ...
  → Redirect to AI CEO dashboard with new team running
```
...
### 5.3 Creator: Export from AI CEO → Publish

```
Running AI CEO company → "Export as Blueprint" (CLI or UI)
  → AI CEO exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (AI CEO brand blue/purple)
...
| Install flow | One-click deploy to AI CEO company |
...
{
  "targetCompanyId": "uuid",         // Existing AI CEO company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `ai-ceo-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as AI CEO
- **Backend**: Node.js API (or extend AI CEO server)
- **Database**: Postgres (can share AI CEO's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with AI CEO auth (or OAuth2)

### 9.2 Integration with AI CEO

ClipHub can be:
- **Option A**: A separate app that calls AI CEO's API to install blueprints
- **Option B**: A built-in section of the AI CEO UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing AI CEO UI and API.
...
- [ ] Install flow: blueprint → AI CEO company
...
- [ ] "Export from AI CEO" CLI command
