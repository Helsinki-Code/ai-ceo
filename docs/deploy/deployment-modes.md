...
AI CEO supports two runtime modes with different security profiles.
...
```sh
pnpm ai-ceo allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, AI CEO emits a one-time claim URL at startup:
...
```sh
AI_CEO_DEPLOYMENT_MODE=authenticated pnpm ai-ceo run
```
