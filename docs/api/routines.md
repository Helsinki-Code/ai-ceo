...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-AI CEO-Signature` + `X-AI CEO-Timestamp` header pair matching the trigger's signing mode.
