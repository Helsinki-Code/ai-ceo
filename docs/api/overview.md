...
AI CEO exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`AI_CEO_API_KEY`)
...
- Run audit trail: include `X-AI CEO-Run-Id` header on all mutating requests during heartbeats
