...
Build a custom adapter to connect AI CEO to any agent runtime.
...
`format-event.ts` — pretty-prints stdout for `ai-ceo run --watch` using `picocolors`.
...
Make AI CEO skills discoverable to your agent runtime without writing to the agent's working directory:
