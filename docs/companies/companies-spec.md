...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only AI CEO.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.ai-ceo.yaml`
...
- AI CEO must not require extra top-level fields for skill validity
- AI CEO-specific extensions must live under `metadata.ai-ceo` or `metadata.sources`
...
For AI CEO, the preferred fidelity extension is:
...
- issue/task AI CEO-only metadata
...
- AI CEO tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested AI CEO shape:
...
Additional rules for AI CEO exporters:
...
- omit empty and default-valued AI CEO fields when possible
...
- AI CEO exporters should emit `.ai-ceo.yaml` as a sidecar by default
...
AI CEO can map this spec to its runtime model like this:
...
- AI CEO extension:
  - `.ai-ceo.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other AI CEO-specific fidelity

Inline AI CEO-only metadata that must live inside a shared markdown file should use:

- `metadata.ai-ceo`

That keeps the base format broader than AI CEO.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only AI CEO.

## 20. Cutover

AI CEO should cut over to this markdown-first package model as the primary portability format.

`ai-ceo.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For AI CEO, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
