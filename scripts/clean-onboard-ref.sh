...
Environment overrides:
  ...
  PC_DATA=/tmp/data           AI CEO data dir to use
  AI_CEO_HOST=127.0.0.1    Host passed to the onboarded server
  AI_CEO_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/ai-ceo-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/ai-ceo-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export AI_CEO_HOST="${AI_CEO_HOST:-127.0.0.1}"
export AI_CEO_PORT="${AI_CEO_PORT:-3100}"
export AI_CEO_OPEN_ON_LISTEN="${AI_CEO_OPEN_ON_LISTEN:-false}"
...
echo "AI_CEO_HOST: $AI_CEO_HOST"
echo "AI_CEO_PORT: $AI_CEO_PORT"
...
env \
  ...
  HOST="$AI_CEO_HOST" \
  PORT="$AI_CEO_PORT" \
  AI_CEO_OPEN_ON_LISTEN="$AI_CEO_OPEN_ON_LISTEN" \
  pnpm ai-ceo onboard --yes --data-dir "$PC_DATA"
