#!/usr/bin/env bash
#
# Kill all local AI CEO dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all ai-ceo dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/ai-ceo(-[^/]+)?/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.ai-ceo/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.ai-ceo/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.ai-ceo/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/ai-ceo*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.ai-ceo/instances/*/db/postmaster.pid
    "$sibling_root"/.ai-ceo/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No AI CEO dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} AI CEO dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to AI CEO node processes..."
