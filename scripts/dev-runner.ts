...
const devServerStatusFilePath = path.join(repoRoot, ".ai-ceo", "dev-server-status.json");
...
const env: NodeJS.ProcessEnv = {
  ...process.env,
  AI_CEO_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.AI_CEO_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.AI_CEO_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.AI_CEO_MIGRATION_PROMPT ??= "never";
  env.AI_CEO_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.AI_CEO_DEPLOYMENT_MODE = "authenticated";
  env.AI_CEO_DEPLOYMENT_EXPOSURE = "private";
  env.AI_CEO_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[ai-ceo] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[ai-ceo] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[ai-ceo] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@ai-ceo/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[ai-ceo] Command failed with code ${status.code}: pnpm --filter @ai-ceo/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[ai-ceo] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.AI_CEO_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[ai-ceo] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[ai-ceo] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@ai-ceo/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[ai-ceo] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@ai-ceo/server", serverScript, ...forwardedArgs],
