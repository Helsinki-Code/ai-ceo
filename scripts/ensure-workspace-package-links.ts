...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".ai-ceo", "dist", "node_modules"]);
...
async function ensureServerWorkspaceLinksCurrent() {
  ...
  console.log("[ai-ceo] detected stale workspace package links for server; relinking dependencies...");
  for (const mismatch of mismatches) {
    console.log(
      `[ai-ceo]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
