...
AI_CEO_HOST_PORT="${AI_CEO_HOST_PORT:-3100}"
AI_CEO_HOST_FROM_CONTAINER="${AI_CEO_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.ai-ceo-openclaw.override.yml"
...
detect_ai_ceo_base_url() {
  ...
  for candidate in "$AI_CEO_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${AI_CEO_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${AI_CEO_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
ai_ceo_base_url="$(detect_ai_ceo_base_url || true)"
...
AI CEO URL for OpenClaw container:
EOF
  if [[ -n "$ai_ceo_base_url" ]]; then
    cat <<EOF
  $ai_ceo_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${AI_CEO_HOST_PORT}
  (Do not use http://127.0.0.1:${AI_CEO_HOST_PORT} inside the container.)
  If AI CEO rejects the host, run on host machine:
    pnpm ai-ceo allowed-hostname host.docker.internal
  Then restart AI CEO and re-run this script.
EOF
  fi
  cat <<EOF
