...
COMPANY_ID="${COMPANY_ID:-${AI_CEO_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-ai-ceo-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-ai-ceo-openclaw-smoke}"
...
Provide one of:
  AI_CEO_AUTH_HEADER=\"Bearer <board-token>\"
  AI_CEO_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "AI CEO OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.ai-ceo.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
