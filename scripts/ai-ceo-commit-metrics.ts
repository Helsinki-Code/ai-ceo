...
const DEFAULT_QUERY = "\"Co-Authored-By: AI CEO <noreply@ai-ceo.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/ai-ceo-commit-metrics-cache.json");
...
const AI_CEO_EMAIL = "noreply@ai-ceo.ing";
const AI_CEO_NAME = "ai-ceo";
...
function printHelp() {
  console.log(`Usage: tsx scripts/ai-ceo-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === AI_CEO_EMAIL) || (displayName && displayName.toLowerCase() === AI_CEO_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("AI CEO commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "ai-ceo-commit-metrics",
