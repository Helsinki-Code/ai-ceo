...
base_cwd="${AI_CEO_WORKSPACE_BASE_CWD:?AI_CEO_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${AI_CEO_WORKSPACE_CWD:?AI_CEO_WORKSPACE_CWD is required}"
ai_ceo_home="${AI_CEO_HOME:-$HOME/.ai-ceo}"
ai_ceo_instance_id="${AI_CEO_INSTANCE_ID:-default}"
ai_ceo_dir="$worktree_cwd/.ai-ceo"
worktree_config_path="$ai_ceo_dir/config.json"
worktree_env_path="$ai_ceo_dir/.env"
worktree_name="${AI_CEO_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${AI_CEO_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.ai-ceo/config.json" || -L "$base_cwd/.ai-ceo/config.json" ) ]]; then
  source_config_path="$base_cwd/.ai-ceo/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$ai_ceo_home/instances/$ai_ceo_instance_id/config.json"
fi
...
run_ai-ceo_command() {
  local command_args=("$@")
  if command -v pnpm >/dev/null 2>&1 && pnpm ai-ceo --help >/dev/null 2>&1; then
    pnpm ai-ceo "${command_args[@]}"
    return 0
  fi
  ...
  if command -v ai-ceo >/dev/null 2>&1; then
    ai-ceo "${command_args[@]}"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  AI_CEO_DIR="$ai_ceo_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  AI_CEO_WORKTREES_DIR="${AI_CEO_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const aiCeoDir = process.env.AI_CEO_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.AI_CEO_WORKTREES_DIR) ?? "~/.ai-ceo-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(aiCeoDir, "config.json");
  const envPath = path.resolve(aiCeoDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "ai-ceo",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.AI_CEO_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.AI_CEO_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.AI_CEO_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "AI_CEO_HOME=" + JSON.stringify(worktreeHome),
    "AI_CEO_INSTANCE_ID=" + JSON.stringify(instanceId),
    "AI_CEO_CONFIG=" + JSON.stringify(configPath),
    "AI_CEO_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "AI_CEO_IN_WORKTREE=true",
    "AI_CEO_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.AI_CEO_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("AI_CEO_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "ai-ceo CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi

disable_seeded_routines() {
  local company_id="${AI_CEO_COMPANY_ID:-}"
  if [[ -z "$company_id" ]]; then
    echo "AI_CEO_COMPANY_ID not set; skipping routine disable post-step." >&2
    return 0
  fi

  if ! run_ai-ceo_command routines disable-all --config "$worktree_config_path" --company-id "$company_id"; then
    echo "ai-ceo CLI not available in this workspace; skipping routine disable post-step." >&2
  fi
}
