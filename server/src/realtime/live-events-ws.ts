...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@ai-ceo/db";
import type { DeploymentMode } from "@ai-ceo/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  aiCeoUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).aiCeoUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.aiCeoUpgradeContext = context;
