...
import { readAICEOSkillSyncPreference } from "@ai-ceo/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@ai-ceo/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@ai-ceo/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@ai-ceo/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveAICEOInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "ai-ceo-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "ai-ceo-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "ai-ceo-feedback-v1";
const FEEDBACK_DESTINATION = "ai_ceo_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "ai_ceo_run" &&
    file.source !== "ai_ceo_run_events" &&
    file.source !== "ai_ceo_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.AI_CEO_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readAICEOSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let aiCeoRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      aiCeoRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "ai-ceo/run.json",
        contentType: "application/json",
        source: "ai_ceo_run",
        contents: `${JSON.stringify(aiCeoRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.aiCeoRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "ai-ceo/run-events.json",
        contentType: "application/json",
        source: "ai_ceo_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "ai-ceo/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "ai_ceo_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.aiCeoRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
