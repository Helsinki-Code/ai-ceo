 ...
 *    (`~/.ai-ceo/plugins/`) and `node_modules` for packages matching
 *    the `ai-ceo-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published AI CEO plugins.
 * Packages matching this pattern are considered AI CEO plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "ai-ceo-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: AICEOPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.ai-ceo/plugins/ local directory
  | "npm"               // npm packages matching ai-ceo-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: AICEOPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the ai-ceo-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "ai-ceo-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: AICEOPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover AI CEO plugins installed as npm packages in the current
   * Node.js environment matching the "ai-ceo-plugin-*" naming convention.
   *
   ...
   * the "aiCeoPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a AI CEO plugin.
   * Throws if the package is a AI CEO plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<AICEOPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: AICEOPluginManifestV1;
    newManifest: AICEOPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the AI CEO plugin naming convention.
 * Accepts both the "ai-ceo-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @ai-ceo/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "aiCeoPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const aiCeoPlugin = pkgJson["aiCeoPlugin"];
  if (
    aiCeoPlugin !== null &&
    typeof aiCeoPlugin === "object" &&
    !Array.isArray(aiCeoPlugin)
  ) {
    const manifestRelPath = (aiCeoPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: AICEOPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumAICEOVersion;
}
...
export function getPluginUiContributionMetadata(
  manifest: AICEOPluginManifestV1,
 ...
 *   packageName: "ai-ceo-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: AICEOPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as AICEOPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a AI CEO plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a AI CEO plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasAICEOPlugin = "aiCeoPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasAICEOPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<AICEOPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: AICEOPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @ai-ceo/shared exports). Run those workers through
