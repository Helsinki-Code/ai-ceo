...
import { companySkills } from "@ai-ceo/db";
import { readAICEOSkillSyncPreference, writeAICEOSkillSyncPreference } from "@ai-ceo/adapter-utils/server-utils";
import type { AICEOSkillEntry } from "@ai-ceo/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@ai-ceo/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveAICEOInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("ai-ceo/ai-ceo/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveAICEOInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "ai_ceo_bundled") {
    return {
      editable: false,
      editableReason: "Bundled AI CEO skills are read-only.",
      sourceLabel: "AI CEO bundled",
      sourceBadge: "ai-ceo",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "ai_ceo_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled AI CEO skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "ai_ceo_bundled"
        && incomingKind === "github"
        && incomingOwner === "ai-ceo"
        && incomingRepo === "ai-ceo"
      ) {
        out.push(existing);
        continue;
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeAICEOSkillSyncPreference(config, filtered),
        });
      }
    }
