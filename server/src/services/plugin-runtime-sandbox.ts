...
import type { AICEOPluginManifestV1 } from "@ai-ceo/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __ai_ceo_exports: module.exports,
      __ai_ceo_module: module,
      __ai_ceo_require: requireInSandbox,
      __ai_ceo_filename: realPath,
      __ai_ceo_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__ai_ceo_exports, __ai_ceo_module, __ai_ceo_require, __ai_ceo_filename, __ai_ceo_dirname)`;
