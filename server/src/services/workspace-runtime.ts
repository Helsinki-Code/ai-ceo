...
import type { AdapterRuntimeServiceReport } from "@ai-ceo/adapter-utils";
import type { Db } from "@ai-ceo/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@ai-ceo/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AI_CEO_WORKSPACE_CWD = input.worktreePath;
  env.AI_CEO_WORKSPACE_PATH = input.worktreePath;
  env.AI_CEO_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AI_CEO_WORKSPACE_BRANCH = input.branchName;
  env.AI_CEO_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AI_CEO_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AI_CEO_WORKSPACE_SOURCE = input.base.source;
  env.AI_CEO_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AI_CEO_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AI_CEO_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AI_CEO_PROJECT_ID = input.base.projectId ?? "";
  env.AI_CEO_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AI_CEO_AGENT_ID = input.agent.id ?? "";
  env.AI_CEO_AGENT_NAME = input.agent.name;
  env.AI_CEO_COMPANY_ID = input.agent.companyId;
  env.AI_CEO_ISSUE_ID = input.issue?.id ?? "";
  env.AI_CEO_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AI_CEO_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AI_CEO_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AI_CEO_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AI_CEO_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AI_CEO_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AI_CEO_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AI_CEO_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AI_CEO_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AI_CEO_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AI_CEO_PROJECT_ID = input.workspace.projectId ?? "";
  env.AI_CEO_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AI_CEO_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".ai-ceo", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "AI CEO", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
