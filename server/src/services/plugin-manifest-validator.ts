 ...
 */
import { pluginManifestV1Schema } from "@ai-ceo/shared";
import type { AICEOPluginManifestV1 } from "@ai-ceo/shared";
import { PLUGIN_API_VERSION } from "@ai-ceo/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): AICEOPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as AICEOPluginManifestV1,
        };
      }
