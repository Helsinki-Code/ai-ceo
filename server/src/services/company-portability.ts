...
import {
  readAICEOSkillSyncPreference,
  writeAICEOSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".ai-ceo.yaml" || normalized === ".ai-ceo.yml") return "extension";
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "ai_ceo_bundled") {
    return `ai-ceo/ai-ceo/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; AI CEO will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .ai-ceo.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[aiCeoExtensionPath];
  if (selectedFiles.has(aiCeoExtensionPath) && typeof extensionEntry === "string") {
    filtered[aiCeoExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findAICEOExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".ai-ceo.yaml"] === "string") return ".ai-ceo.yaml";
  if (typeof files[".ai-ceo.yml"] === "string") return ".ai-ceo.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.ai-ceo.yaml") || entry.endsWith("/.ai-ceo.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "ai_ceo_bundled") {
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "ai_ceo_bundled") return true;
...
function buildManifestFromPackageFiles(
  ...
  const aiCeoExtensionPath = findAICEOExtensionPath(normalizedFiles);
  const aiCeoExtension = aiCeoExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, aiCeoExtensionPath) ?? "")
    : {};
  const aiCeoCompany = isPlainRecord(aiCeoExtension.company) ? aiCeoExtension.company : {};
  const aiCeoSidebar = normalizePortableSidebarOrder(aiCeoExtension.sidebar);
  const aiCeoAgents = isPlainRecord(aiCeoExtension.agents) ? aiCeoExtension.agents : {};
  const aiCeoProjects = isPlainRecord(aiCeoExtension.projects) ? aiCeoExtension.projects : {};
  const aiCeoTasks = isPlainRecord(aiCeoExtension.tasks) ? aiCeoExtension.tasks : {};
  const aiCeoRoutines = isPlainRecord(aiCeoExtension.routines) ? aiCeoExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(aiCeoCompany.brandColor),
      logoPath: asString(aiCeoCompany.logoPath) ?? asString(aiCeoCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof aiCeoCompany.requireBoardApprovalForNewAgents === "boolean"
          ? aiCeoCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof aiCeoCompany.feedbackDataSharingEnabled === "boolean"
          ? aiCeoCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof aiCeoCompany.feedbackDataSharingConsentAt === "string"
          ? aiCeoCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(aiCeoCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(aiCeoCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: aiCeoSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(aiCeoAgents[slug]) ? aiCeoAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(aiCeoProjects[slug]) ? aiCeoProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(aiCeoTasks[slug]) ? aiCeoTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(aiCeoRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(aiCeoRoutines[slug]) ? aiCeoRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const aiCeoAgentsOut: Record<string, Record<string, unknown>> = {};
    const aiCeoProjectsOut: Record<string, Record<string, unknown>> = {};
    const aiCeoTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const aiCeoRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readAICEOSkillSyncPreference(
        ...
        aiCeoAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      aiCeoProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      aiCeoTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      aiCeoRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const aiCeoExtensionPath = ".ai-ceo.yaml";
    const aiCeoAgents = Object.fromEntries(
      Object.entries(aiCeoAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aiCeoProjects = Object.fromEntries(
      Object.entries(aiCeoProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aiCeoTasks = Object.fromEntries(
      Object.entries(aiCeoTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aiCeoRoutines = Object.fromEntries(
      Object.entries(aiCeoRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[aiCeoExtensionPath] = buildYamlFile(
      {
        schema: "ai-ceo/v1",
        ...
        agents: Object.keys(aiCeoAgents).length > 0 ? aiCeoAgents : undefined,
        projects: Object.keys(aiCeoProjects).length > 0 ? aiCeoProjects : undefined,
        tasks: Object.keys(aiCeoTasks).length > 0 ? aiCeoTasks : undefined,
        routines: Object.keys(aiCeoRoutines).length > 0 ? aiCeoRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, aiCeoExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeAICEOSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
