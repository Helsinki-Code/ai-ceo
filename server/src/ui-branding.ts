const FAVICON_BLOCK_START = "<!-- AI_CEO_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- AI_CEO_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- AI_CEO_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- AI_CEO_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.AI_CEO_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.AI_CEO_WORKTREE_NAME) ?? nonEmpty(env.AI_CEO_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.AI_CEO_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="ai-ceo-worktree-enabled" content="true" />',
    `<meta name="ai-ceo-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="ai-ceo-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="ai-ceo-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
