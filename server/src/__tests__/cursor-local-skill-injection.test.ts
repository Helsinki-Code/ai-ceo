...
import { ensureCursorSkillsInjected } from "@ai-ceo/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing AI CEO skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("ai-ceo-cursor-skills-src-");
    const skillsHome = await makeTempDir("ai-ceo-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "ai-ceo");
    await createSkillDir(skillsDir, "ai-ceo-create-agent");
    ...
    const injectedA = path.join(skillsHome, "ai-ceo");
    const injectedB = path.join(skillsHome, "ai-ceo-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "ai-ceo")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "ai-ceo-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "ai-ceo"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "ai-ceo-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("ai-ceo-cursor-preserve-src-");
    const skillsHome = await makeTempDir("ai-ceo-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "ai-ceo");
    ...
    expect((await fs.lstat(path.join(skillsHome, "ai-ceo-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("ai-ceo-cursor-fail-src-");
    const skillsHome = await makeTempDir("ai-ceo-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
