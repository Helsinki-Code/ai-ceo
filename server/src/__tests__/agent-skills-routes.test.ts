...
describe("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["ai-ceo/ai-ceo/ai-ceo"],
      entries: [],
      warnings: [],
    });
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          aiCeoRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  });
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await request(createApp())
      ...
      .send({ desiredSkills: ["ai-ceo/ai-ceo/ai-ceo"] });
  ...
  it("canonicalizes desired skill references before syncing", async () => {
    ...
    const res = await request(createApp())
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["ai-ceo"] });

    expect(res.status, JSON.stringify(res.body)).toBe(200);
    expect(mockCompanySkillService.resolveRequestedSkillKeys).toHaveBeenCalledWith("company-1", ["ai-ceo"]);
    expect(mockAgentService.update).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          aiCeoSkillSync: expect.objectContaining({
            desiredSkills: ["ai-ceo/ai-ceo/ai-ceo"],
          }),
        }),
      }),
