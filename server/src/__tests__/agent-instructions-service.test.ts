...
describe("agent instructions service", () => {
  const originalAICEOHome = process.env.AI_CEO_HOME;
  const originalAICEOInstanceId = process.env.AI_CEO_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalAICEOHome === undefined) delete process.env.AI_CEO_HOME;
    else process.env.AI_CEO_HOME = originalAICEOHome;
    if (originalAICEOInstanceId === undefined) delete process.env.AI_CEO_INSTANCE_ID;
    else process.env.AI_CEO_INSTANCE_ID = originalAICEOInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const aiCeoHome = await makeTempDir("ai-ceo-agent-instructions-home-");
    const externalRoot = await makeTempDir("ai-ceo-agent-instructions-external-");
    cleanupDirs.add(aiCeoHome);
    cleanupDirs.add(externalRoot);
    process.env.AI_CEO_HOME = aiCeoHome;
    process.env.AI_CEO_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("ai-ceo-agent-instructions-new-external-");
    cleanupDirs.add(aiCeoHome);
    cleanupDirs.add(externalRoot);
    process.env.AI_CEO_HOME = aiCeoHome;
    process.env.AI_CEO_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("ai-ceo-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const aiCeoHome = await makeTempDir("ai-ceo-agent-instructions-recover-");
    cleanupDirs.add(aiCeoHome);
    process.env.AI_CEO_HOME = aiCeoHome;
    process.env.AI_CEO_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const aiCeoHome = await makeTempDir("ai-ceo-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("ai-ceo-agent-instructions-stale-root-");
    cleanupDirs.add(aiCeoHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const aiCeoHome = await makeTempDir("ai-ceo-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("ai-ceo-agent-instructions-heal-write-stale-");
    cleanupDirs.add(aiCeoHome);
    cleanupDirs.add(staleRoot);
    process.env.AI_CEO_HOME = aiCeoHome;
    process.env.AI_CEO_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const aiCeoHome = await makeTempDir("ai-ceo-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("ai-ceo-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(aiCeoHome);
    cleanupDirs.add(staleRoot);
    process.env.AI_CEO_HOME = aiCeoHome;
    process.env.AI_CEO_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const aiCeoHome = await makeTempDir("ai-ceo-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("ai-ceo-agent-instructions-partial-root-");
    cleanupDirs.add(aiCeoHome);
    cleanupDirs.add(staleRoot);
    process.env.AI_CEO_HOME = aiCeoHome;
    process.env.AI_CEO_INSTANCE_ID = "test-instance";
