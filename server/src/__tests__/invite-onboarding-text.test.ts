...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("AI CEO OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested AI CEO base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("aiCeoApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.aiCeoApiUrl");
    expect(text).toContain("~/.openclaw/workspace/ai-ceo-claimed-api-key.json");
    expect(text).toContain("AI_CEO_API_KEY");
