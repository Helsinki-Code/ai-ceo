...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "ai-ceo/ai-ceo/ai-ceo-create-agent";
  ...
  it("defaults to mounting all built-in AI CEO skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === aiCeoKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat AI CEO skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("ai-ceo-claude-user-skills-");
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      readOnly: true,
      detail: "Installed outside AI CEO management in the Claude skills home.",
    }));
  });
});
