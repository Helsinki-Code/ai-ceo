...
<head>
    <!-- AI_CEO_RUNTIME_BRANDING_START -->
    <!-- AI_CEO_RUNTIME_BRANDING_END -->
    <!-- AI_CEO_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from AI_CEO_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ AI_CEO_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ AI_CEO_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ AI_CEO_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      AI_CEO_IN_WORKTREE: "true",
      AI_CEO_WORKTREE_NAME: "ai-ceo-pr-432",
      AI_CEO_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("ai-ceo-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="ai-ceo-worktree-name"');
    expect(meta).toContain('content="ai-ceo-pr-432"');
    expect(meta).toContain('name="ai-ceo-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="ai-ceo-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="ai-ceo-worktree-name"');
  });
});
