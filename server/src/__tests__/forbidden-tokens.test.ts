...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "ai-ceo", LOGNAME: "ai-ceo", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "ai-ceo" }),
      },
    );

    expect(tokens).toEqual(["ai-ceo", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "ai-ceo" }, {
        userInfo: () => ({ username: "ai-ceo" }),
      });

      expect(tokens).toEqual(["ai-ceo", "custom-token"]);
    } finally {
