...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ai-ceo-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const aiCeoDir = path.join(worktreeRoot, ".ai-ceo");
    const configPath = path.join(aiCeoDir, "config.json");
    const envPath = path.join(aiCeoDir, ".env");
    const sharedRoot = path.join(tempRoot, ".ai-ceo", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".ai-ceo-worktrees");

    await fs.mkdir(aiCeoDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# AI CEO environment variables",
        "AI_CEO_IN_WORKTREE=true",
        "AI_CEO_WORKTREE_NAME=PAP-884-ai-commits-component",
        "AI_CEO_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.AI_CEO_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.AI_CEO_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`AI_CEO_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('AI_CEO_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`AI_CEO_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`AI_CEO_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('AI_CEO_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.AI_CEO_HOME).toBe(isolatedHome);
    expect(process.env.AI_CEO_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ai-ceo-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# AI CEO environment variables",
        "AI_CEO_IN_WORKTREE=true",
        "AI_CEO_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.AI_CEO_IN_WORKTREE = "true";
    process.env.AI_CEO_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.AI_CEO_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ai-ceo-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".ai-ceo-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".ai-ceo", "worktrees");
    ...
    const aiCeoDir = path.join(currentWorktreeRoot, ".ai-ceo");
    const configPath = path.join(aiCeoDir, "config.json");
    const envPath = path.join(aiCeoDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".ai-ceo", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ai-ceo-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.AI_CEO_IN_WORKTREE = "true";
    process.env.AI_CEO_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.AI_CEO_HOME = isolatedHome;
    process.env.AI_CEO_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.AI_CEO_CONFIG = configPath;
