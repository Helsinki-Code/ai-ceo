...
describe("agent local JWT", () => {
  const secretEnv = "AI_CEO_AGENT_JWT_SECRET";
  const ttlEnv = "AI_CEO_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "AI_CEO_AGENT_JWT_ISSUER";
  const audienceEnv = "AI_CEO_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "ai-ceo";
    process.env[audienceEnv] = "ai-ceo-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
