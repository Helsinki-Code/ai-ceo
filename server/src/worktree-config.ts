...
import type { AICEOConfig } from "@ai-ceo/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by AI CEO worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.AI_CEO_IN_WORKTREE !== "true") return null;

  const configPath = resolveAICEOConfigPath(overrideConfigPath);
  const envPath = resolveAICEOEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.AI_CEO_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.AI_CEO_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.AI_CEO_HOME) ??
      nonEmpty(env.AI_CEO_WORKTREES_DIR) ??
      "~/.ai-ceo-worktrees",
  );
...
function writeConfigFile(configPath: string, config: AICEOConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".ai-ceo", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as AICEOConfig;
...
): { config: AICEOConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.AI_CEO_HOME = context.homeDir;
  process.env.AI_CEO_INSTANCE_ID = context.instanceId;
  process.env.AI_CEO_CONFIG = context.configPath;
  process.env.AI_CEO_CONTEXT = context.contextPath;
  process.env.AI_CEO_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AICEOConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    AI_CEO_HOME: context.homeDir,
    AI_CEO_INSTANCE_ID: context.instanceId,
    AI_CEO_CONFIG: context.configPath,
    AI_CEO_CONTEXT: context.contextPath,
    AI_CEO_IN_WORKTREE: "true",
    AI_CEO_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: AICEOConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AICEOConfig;
  } catch {
    return;
  }
