...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@ai-ceo/db";
...
import { runClaudeLogin } from "@ai-ceo/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@ai-ceo/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@ai-ceo/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@ai-ceo/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@ai-ceo/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.AI_CEO_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeAICEOSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readAICEOSkillSyncPreference(
