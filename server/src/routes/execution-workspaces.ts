...
import { issues, projects, projectWorkspaces } from "@ai-ceo/db";
import { updateExecutionWorkspaceSchema } from "@ai-ceo/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before AI CEO can manage local runtime services" });
      return;
    }
