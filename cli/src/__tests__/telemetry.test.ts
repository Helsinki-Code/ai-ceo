...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "ai-ceo-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.AI_CEO_HOME = path.join(root, "home");
    process.env.AI_CEO_INSTANCE_ID = "telemetry-test";
