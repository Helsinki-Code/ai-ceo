...
import { ApiConnectionError, ApiRequestError, AICEOApiClient } from "../client/http.js";

describe("AICEOApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new AICEOApiClient({
    ...
    expect(headers["x-ai-ceo-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new AICEOApiClient({ apiBase: "http://localhost:3100" });
