...
export class AICEOApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-ai-ceo-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the AI CEO API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the AI CEO server is not running, the configured URL is wrong, or the request is being blocked before it reaches AI CEO.",
    "",
    "Try:",
    "- Start AI CEO with `pnpm dev` or `pnpm ai-ceo run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If AI CEO is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`AI_CEO_API_URL\`.`,
  );
  return lines.join("\n");
}
