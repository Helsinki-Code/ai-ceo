...
import { ApiRequestError, AICEOApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to AI CEO config file")
    .option("-d, --data-dir <path>", "AI CEO data directory root (isolates state from ~/.ai-ceo)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the AI CEO API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AI_CEO_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.AI_CEO_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.AI_CEO_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AI_CEO_COMPANY_ID, or set context profile companyId via `ai-ceo context set`.",
    );
  }

  const api = new AICEOApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AI_CEO_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AI_CEO_SERVER_PORT || "");
