...
const DEFAULT_AGENT_JWT_ISSUER = "ai-ceo";
const DEFAULT_AGENT_JWT_AUDIENCE = "ai-ceo-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveAICEOInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveAICEOInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" ai-ceo env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: AICEOConfig | null = null;
...
function collectDeploymentEnvRows(config: AICEOConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    process.env.AI_CEO_PUBLIC_URL ??
    process.env.AI_CEO_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.AI_CEO_PUBLIC_URL
      ? "env"
      : process.env.AI_CEO_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from AI_CEO_PUBLIC_URL when possible)",
    },
    {
      key: "AI_CEO_AGENT_JWT_TTL_SECONDS",
      value: process.env.AI_CEO_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.AI_CEO_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "AI_CEO_AGENT_JWT_ISSUER",
      value: process.env.AI_CEO_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.AI_CEO_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "AI_CEO_AGENT_JWT_AUDIENCE",
      value: process.env.AI_CEO_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.AI_CEO_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "AI_CEO_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.AI_CEO_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.AI_CEO_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "AI_CEO_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.AI_CEO_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.AI_CEO_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.AI_CEO_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.AI_CEO_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.AI_CEO_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.AI_CEO_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.AI_CEO_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "AI_CEO_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.AI_CEO_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.AI_CEO_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "AI_CEO_CONFIG",
      value: process.env.AI_CEO_CONFIG ?? configPath,
      source: process.env.AI_CEO_CONFIG ? "env" : "default",
      required: false,
