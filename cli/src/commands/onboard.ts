...
type OnboardDefaults = Pick<AICEOConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "AI_CEO_PUBLIC_URL",
  "DATABASE_URL",
  "AI_CEO_DB_BACKUP_ENABLED",
  "AI_CEO_DB_BACKUP_INTERVAL_MINUTES",
  "AI_CEO_DB_BACKUP_RETENTION_DAYS",
  "AI_CEO_DB_BACKUP_DIR",
  "AI_CEO_DEPLOYMENT_MODE",
  "AI_CEO_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "AI_CEO_ALLOWED_HOSTNAMES",
  "AI_CEO_AUTH_BASE_URL_MODE",
  "AI_CEO_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "AI_CEO_STORAGE_PROVIDER",
  "AI_CEO_STORAGE_LOCAL_DIR",
  "AI_CEO_STORAGE_S3_BUCKET",
  "AI_CEO_STORAGE_S3_REGION",
  "AI_CEO_STORAGE_S3_ENDPOINT",
  "AI_CEO_STORAGE_S3_PREFIX",
  "AI_CEO_STORAGE_S3_FORCE_PATH_STYLE",
  "AI_CEO_SECRETS_PROVIDER",
  "AI_CEO_SECRETS_STRICT_MODE",
  "AI_CEO_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveAICEOInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.AI_CEO_PUBLIC_URL?.trim() ||
    process.env.AI_CEO_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.AI_CEO_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.AI_CEO_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.AI_CEO_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.AI_CEO_ALLOWED_HOSTNAMES
    ? process.env.AI_CEO_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.AI_CEO_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.AI_CEO_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.AI_CEO_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.AI_CEO_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.AI_CEO_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.AI_CEO_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.AI_CEO_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.AI_CEO_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.AI_CEO_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.AI_CEO_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.AI_CEO_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.AI_CEO_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.AI_CEO_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.AI_CEO_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.AI_CEO_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "AI_CEO_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<AICEOConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printAICEOCliBanner();
  p.intro(pc.bgCyan(pc.black(" ai-ceo onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveAICEOInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: AICEOConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing AI CEO install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("ai-ceo configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("AI_CEO_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.AI_CEO_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("AI_CEO_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("AI_CEO_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: AI_CEO_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("ai-ceo run")}`,
        `Reconfigure later: ${pc.cyan("ai-ceo configure")}`,
        `Diagnose setup: ${pc.cyan("ai-ceo doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.AI_CEO_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing AI CEO setup is ready.");
    return;
  }
  ...
  let llm: AICEOConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@ai-ceo/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `ai-ceo doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("ai-ceo run")}`,
        `Then: ${pc.cyan("ai-ceo auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
