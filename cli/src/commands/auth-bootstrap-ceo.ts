...
import { createDb, instanceUserRoles, invites } from "@ai-ceo/db";
...
export async function bootstrapCeoInvite(opts: {
  ...
  const config = readConfig(configPath);
  if (!config) {
    p.log.error(`No config found at ${configPath}. Run ${pc.cyan("ai-ceo onboard")} first.`);
    return;
  }
  ...
  try {
    ...
    p.log.info("If using embedded-postgres, start the AI CEO server and run this command again.");
  } finally {
