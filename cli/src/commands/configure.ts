...
function defaultConfig(): AICEOConfig {
...
export async function configure(opts: {
  ...
  printAICEOCliBanner();
  p.intro(pc.bgCyan(pc.black(" ai-ceo configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `ai-ceo onboard` first.");
    p.outro("");
    return;
  }

  let config: AICEOConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because AI_CEO_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
