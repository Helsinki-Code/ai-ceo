...
export const DEFAULT_WORKTREE_HOME = "~/.ai-ceo-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".ai-ceo");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "ai-ceo",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    AI_CEO_HOME: paths.homeDir,
    AI_CEO_INSTANCE_ID: paths.instanceId,
    AI_CEO_CONFIG: paths.configPath,
    AI_CEO_CONTEXT: paths.contextPath,
    AI_CEO_IN_WORKTREE: "true",
    ...(branding?.name ? { AI_CEO_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { AI_CEO_WORKTREE_COLOR: branding.color } : {}),
  };
}
