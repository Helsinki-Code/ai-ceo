...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@ai-ceo/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://ai-ceo:ai-ceo@127.0.0.1:${port}/ai-ceo`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printAICEOCliBanner();
  p.intro(pc.bgCyan(pc.black(" ai-ceo db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveAICEOInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "ai-ceo";
