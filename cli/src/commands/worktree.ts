...
import { ensureAgentJwtSecret, loadAICEOEnvFile, mergeAICEOEnvEntries, readAICEOEnvEntries, resolveAICEOEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { AICEOConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printAICEOCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.AI_CEO_CONFIG;
...
const WORKTREE_NAME_PREFIX = "ai-ceo-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.AI_CEO_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.AI_CEO_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromAICEOConfig(config: AICEOConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromAICEOConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.ai-ceo${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".ai-ceo", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".ai-ceo", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.ai-ceo"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: AICEOConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://ai-ceo:ai-ceo@127.0.0.1:${port}/ai-ceo`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: AICEOConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.AI_CEO_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AI_CEO_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.AI_CEO_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AI_CEO_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: AICEOConfig;
  targetConfig: AICEOConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveAICEOEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readAICEOEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://ai-ceo:ai-ceo@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "ai-ceo");
    const targetConnectionString = `postgres://ai-ceo:ai-ceo@127.0.0.1:${targetHandle.port}/ai-ceo`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readAICEOEnvEntries(resolveAICEOEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.AI_CEO_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.AI_CEO_AGENT_JWT_SECRET);
  mergeAICEOEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { AI_CEO_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadAICEOEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run AI CEO inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printAICEOCliBanner();
  p.intro(pc.bgCyan(pc.black(" ai-ceo worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printAICEOCliBanner();
  p.intro(pc.bgCyan(pc.black(" ai-ceo worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasAICEOConfig: existsSync(path.resolve(worktreePath, ".ai-ceo", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printAICEOCliBanner();
  p.intro(pc.bgCyan(pc.black(" ai-ceo worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveAICEOEnvFile(configPath);
  const envEntries = readAICEOEnvEntries(envPath);
  const out = {
    AI_CEO_CONFIG: configPath,
    ...(envEntries.AI_CEO_HOME ? { AI_CEO_HOME: envEntries.AI_CEO_HOME } : {}),
    ...(envEntries.AI_CEO_INSTANCE_ID ? { AI_CEO_INSTANCE_ID: envEntries.AI_CEO_INSTANCE_ID } : {}),
    ...(envEntries.AI_CEO_CONTEXT ? { AI_CEO_CONTEXT: envEntries.AI_CEO_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasAICEOConfig)
      .map((choice) => path.resolve(choice.worktree, ".ai-ceo", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readAICEOEnvEntries(resolveAICEOEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start AI CEO once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasAICEOConfig ? "ai-ceo" : "no-ai-ceo-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".ai-ceo", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".ai-ceo", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .ai-ceo/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasAICEOConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a AI CEO worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasAICEOConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No AI CEO worktrees were found. Run `ai-ceo worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target AI CEO configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local AI CEO instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated AI CEO instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with ai-ceo- if needed (created at ~/ai-ceo-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: AI_CEO_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: AI_CEO_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source AI_CEO_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local AI CEO instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like AI CEO worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with ai-ceo- if needed")
