...
const JWT_SECRET_ENV_KEY = "AI_CEO_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by AI CEO CLI commands",
...
export function resolveAICEOEnvFile(configPath?: string): string {
...
export function loadAICEOEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAICEOEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAICEOEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAICEOEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAICEOEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAICEOEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAICEOEnvEntries(next, filePath);
  return next;
}
