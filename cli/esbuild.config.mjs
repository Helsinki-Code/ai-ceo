/**
 * esbuild configuration for building the ai-ceo CLI for npm.
 *
 * Bundles all workspace packages (@ai-ceo/*) into a single file.
...
// to npm and resolved at runtime (e.g. @ai-ceo/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@ai-ceo/")) {
      externals.add(name);
    }
  }
