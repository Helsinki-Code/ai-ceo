/**
 * Core types for the AI CEO plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@ai-ceo/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @ai-ceo/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of AI CEO object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the AI CEO UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit AI CEO domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core AI CEO domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the AI CEO secret provider
...
export interface PluginSecretsClient {
   ...
   * in the AI CEO secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@ai-ceo/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@ai-ceo/plugin-sdk";
 *
