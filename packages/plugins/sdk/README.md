...
Official TypeScript SDK for AI CEO plugin authors.

- **Worker SDK:** `@ai-ceo/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@ai-ceo/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@ai-ceo/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@ai-ceo/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@ai-ceo/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@ai-ceo/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@ai-ceo/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@ai-ceo/plugin-sdk/ui/hooks` | Hooks only |
| `@ai-ceo/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@ai-ceo/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@ai-ceo/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@ai-ceo/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@ai-ceo/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@ai-ceo/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main AI CEO app. They can call ordinary AI CEO HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the AI CEO instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@ai-ceo/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@ai-ceo/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core AI CEO information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@ai-ceo/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@ai-ceo/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@ai-ceo/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@ai-ceo/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@ai-ceo/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@ai-ceo/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@ai-ceo/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@ai-ceo/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@ai-ceo/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@ai-ceo/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@ai-ceo/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@ai-ceo/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@ai-ceo/plugin-sdk/dev-server";
