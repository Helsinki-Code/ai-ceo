...
type AICEOSprite = (typeof AI_CEO_SPRITES)[number];
...
function spriteSize(sprite: AICEOSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AI_CEO_SPRITES[(baseCol + baseRow) % AI_CEO_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AI_CEO_SPRITES[Math.floor(Math.random() * AI_CEO_SPRITES.length)]!;
