...
Example AI CEO plugin that demonstrates:
...
```bash
pnpm --filter @ai-ceo/plugin-file-browser-example build
pnpm ai-ceo plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm ai-ceo plugin uninstall paperclip-file-browser-example --force
```
