...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".ai-ceo-sdk");
...
/**
 * Generate a complete AI CEO plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A AI CEO plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "ai-ceo-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["ai-ceo", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@ai-ceo/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@ai-ceo/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { AICEOPluginManifestV1 } from "@ai-ceo/plugin-sdk";

const manifest: AICEOPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@ai-ceo/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@ai-ceo/plugin-sdk\` and \`@ai-ceo/shared\` from a local AI CEO checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.ai-ceo-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-ai-ceo-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <ai-ceo-sdk-path>]");
    process.exit(1);
  }
