...
function resolveAICEOInstanceId(): string {
  const raw = process.env.AI_CEO_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveAICEOHomeDir(), "instances", resolveAICEOInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveAICEOHomeDir(), "instances", resolveAICEOInstanceId(), "data", "backups");
}
