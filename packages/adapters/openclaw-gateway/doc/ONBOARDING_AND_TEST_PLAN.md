...
## Scope
This plan is now **gateway-only**. AI CEO supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses AI CEO skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a AI CEO task.
...
```bash
pnpm --filter @ai-ceo/server test:run -- openclaw-gateway
pnpm --filter @ai-ceo/server typecheck
pnpm --filter @ai-ceo/ui typecheck
pnpm --filter ai-ceo typecheck
```
