...
import { asString, parseObject } from "@ai-ceo/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the AI CEO server host.",
        });
      }
    } catch (err) {
      checks.push({
