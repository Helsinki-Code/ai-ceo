...
import { asNumber, asString, buildAICEOEnv, parseObject } from "@ai-ceo/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "ai-ceo";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "ai-ceo";
  if (input.strategy === "run") return `ai-ceo:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `ai-ceo:issue:${input.issueId}`;
  return fallback;
}
...
function resolveAICEOApiUrlOverride(value: unknown): string | null {
...
function buildAICEOEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const aiCeoApiUrlOverride = resolveAICEOApiUrlOverride(ctx.config.aiCeoApiUrl);
  const aiCeoEnv: Record<string, string> = {
    ...buildAICEOEnv(ctx.agent),
    AI_CEO_RUN_ID: ctx.runId,
  };

  if (aiCeoApiUrlOverride) {
    aiCeoEnv.AI_CEO_API_URL = aiCeoApiUrlOverride;
  }
  if (wakePayload.taskId) aiCeoEnv.AI_CEO_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) aiCeoEnv.AI_CEO_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) aiCeoEnv.AI_CEO_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) aiCeoEnv.AI_CEO_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) aiCeoEnv.AI_CEO_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    aiCeoEnv.AI_CEO_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return aiCeoEnv;
}

function buildWakeText(payload: WakePayload, aiCeoEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/ai-ceo-claimed-api-key.json";
  const orderedKeys = [
    "AI_CEO_RUN_ID",
    "AI_CEO_AGENT_ID",
    "AI_CEO_COMPANY_ID",
    ...
    "AI_CEO_TASK_ID",
    "AI_CEO_WAKE_REASON",
    "AI_CEO_WAKE_COMMENT_ID",
    "AI_CEO_APPROVAL_ID",
    "AI_CEO_APPROVAL_STATUS",
    "AI_CEO_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = aiCeoEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = aiCeoEnv.AI_CEO_API_URL ?? "<set AI_CEO_API_URL>";

  const lines = [
    "AI CEO wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `AI_CEO_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load AI_CEO_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $AI_CEO_API_KEY on every API call.",
    "- Use X-AI CEO-Run-Id: $AI_CEO_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: AI_CEO_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$AI_CEO_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$AI_CEO_COMPANY_ID/issues?assigneeAgentId=$AI_CEO_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardAICEOPayload(
  ...
  const runtimeServiceIntents = Array.isArray(ctx.context.aiCeoRuntimeServiceIntents)
    ? ctx.context.aiCeoRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardAiCeo: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: aiCeoEnv.AI_CEO_API_URL ?? null,
  };

  if (workspace) {
    standardAiCeo.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardAiCeo.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardAiCeo.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "ai-ceo-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const aiCeoEnv = buildAICEOEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, aiCeoEnv);
  ...
  const aiCeoPayload = buildStandardAICEOPayload(ctx, wakePayload, aiCeoEnv, payloadTemplate);
