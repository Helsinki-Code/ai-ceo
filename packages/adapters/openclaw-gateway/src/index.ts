...
Use when:
- You want AI CEO to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the AI CEO server.
...
- aiCeoApiUrl (string, optional): absolute AI CEO base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default ai-ceo)

Standard outbound payload additions:
- ai-ceo (object): standardized AI CEO context added to every gateway agent request
- ai-ceo.workspace (object, optional): resolved execution workspace for this run
- ai-ceo.workspaces (array, optional): additional workspace hints AI CEO exposed to the run
- ai-ceo.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
