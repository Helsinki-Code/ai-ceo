...
Use when:
- You want AI CEO to run Cursor Agent CLI locally as the agent runtime
...
- AI CEO auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$ai-ceo" and related skills on local runs.
- AI CEO auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
