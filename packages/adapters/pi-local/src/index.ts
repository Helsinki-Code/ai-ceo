...
Use when:
- You want AI CEO to run Pi (the AI coding agent) locally as the agent runtime
...
- AI CEO requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/ai-ceo-sessions/ and resumed with --session.
