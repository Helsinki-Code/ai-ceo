...
const AI_CEO_SESSIONS_DIR = path.join(os.homedir(), ".pi", "ai-ceo-sessions");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[ai-ceo] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[ai-ceo] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[ai-ceo] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(AI_CEO_SESSIONS_DIR, { recursive: true });
  return AI_CEO_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(AI_CEO_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readAICEORuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveAICEODesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.AI_CEO_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[ai-ceo] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[ai-ceo] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add AI CEO skills directory so Pi can load the ai-ceo skill
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[ai-ceo] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
