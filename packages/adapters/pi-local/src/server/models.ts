...
import type { AdapterModel } from "@ai-ceo/adapter-utils";
import { asString, runChildProcess } from "@ai-ceo/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AI_CEO_PI_COMMAND === "string" &&
    process.env.AI_CEO_PI_COMMAND.trim().length > 0
      ? process.env.AI_CEO_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AI_CEO_", "npm_", "NPM_"] as const;
