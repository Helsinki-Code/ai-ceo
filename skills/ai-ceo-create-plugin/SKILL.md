---
name: ai-ceo-create-plugin
description: >
  Create new AI CEO plugins with the current alpha SDK/runtime. Use when
...
# Create a AI CEO Plugin

Use this skill when the task is to create, scaffold, or document a AI CEO plugin.
...
node packages/plugins/create-ai-ceo-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the AI CEO repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.ai-ceo-sdk/`:

```bash
pnpm --filter @ai-ceo/create-ai-ceo-plugin build
node packages/plugins/create-ai-ceo-plugin/dist/index.js @acme/plugin-name \
...
- is installed into AI CEO from an absolute local path during development
